...
export class PatientsComponent implements OnInit {
  ...
  public onDataEdit(args: DialogEditEventArgs): void {
    if (args.requestType === 'beginEdit') {
      ...
      const fields: Array<string> = ['Id', 'Name', 'Gender', 'DOB', 'BloodGroup', 'Mobile', 'Email', 'Subject'];
  ...
  public getHistoryDetails(): HTMLElement {
    ...
    if (filteredData.length > 0) {
      filteredData.map((item: Record<string, any>) => {
        ...
        element.textContent = `${this.intl.formatDate(item['StartTime'], { skeleton: 'MMMd' })} - ${this.intl.formatDate(item['StartTime'], { skeleton: 'hm' })} - ${this.intl.formatDate(item['EndTime'], { skeleton: 'hm' })} Appointment with Prof.${this.getDoctorName(item['DoctorId'])}`;
