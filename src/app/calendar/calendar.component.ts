...
export class CalendarComponent implements OnInit {
  ...
  public itemTemplate: string = '<div class="specialist-item"><img class="value" src="./assets/images/${Text}.png" alt="teacher"/>' +
    '<div class="doctor-details"><div class="name">Prof.${Name}</div><div class="designation">${Designation}</div></div></div>';
  public footerTemplate = `<div class="add-doctor"><div class="e-icon-add e-icons"></div>
    <div class="add-doctor-text">Add New Teacher</div></div>`;
    ...
    this.eventSettings = {
      ...
      fields: {
        subject: {
          name: 'Name',
          validation: {
            required: [true, 'Enter valid Student Name'],
            range: [this.nameValidation, 'Entered student name is not present, please add new student or select from list']
          }
        },
  ...
  public onActionBegin(args: ActionEventArgs): void {
    if (args.requestType === 'eventCreate' || args.requestType === 'eventChange') {
      ...
      const activityData: Record<string, any> = {
        ...
        Message: `${data['Name']} for ${data['Subject']}`,
        Time: '5 mins ago',
        Type: 'appointment',
  ...
  public getDoctorName(data: Record<string, any>): string {
    if (!isNullOrUndefined(data['DoctorId'])) {
      return 'Prof. ' + this.doctorsData.filter((item: Record<string, any>) => item['Id'] === data['DoctorId'])[0]['Name'].toString();
    } else {
      ...
      if (scheduleElement) {
        ...
        if (event.target.classList.contains('e-work-cells')) {
          ...
          const eventData: Record<string, any> = {
            ...
            Subject: filteredData[0]['Disease'] || filteredData[0]['Subject'],
